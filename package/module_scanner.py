...
# parts of ST3
sys.path.append('gui')
import gui.gui_ModuleScanner as ModuleScanner
...
class ModuleScanner(QDialog, ModuleScanner.Ui_ModuleScanner):
    ...
    def __init__(self):
        ...
        self.pbGo.clicked.connect( self.close )
        self.pbGo.setShortcut(Qt.Key_Return)

    #__________________________________________________________________________
    def do_scan_feb(self, feb_type):
        ''' run external FEB A scanner script'''
        process = QProcess(self)
        process.start('python3', ['scripts/scan_feb_number.py'])
    ...
    def do_scan_module_qr(self):
        ...
        process = QProcess(self)
        process.start('python3', ['scripts/scan_module_qrcode.py'])
        process.waitForFinished()
    ...
    def do_scan_sensor_bar(self):
        ''' run external QR scanner script'''
        process = QProcess(self)
        process.start('python3', ['scripts/scan_sensor_barcode.py'])
        process.waitForFinished()
